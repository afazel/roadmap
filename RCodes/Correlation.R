                                             ...
                                             for(Cindex in 1:ncol(cor)){
                                                 if(colnames(cor)[Cindex]==vector2){
                                                    cor[Rindex,Cindex]<-corr
                                                     mypath <- file.path("Desktop",paste("plot_",nplot, ".png", sep = ""))
                                                     nplot<-nplot+1
                                                     png(file=mypath,width=2000, height=1000) 
                                                     par(mai=c(3,3,3,3)) 
                                                     plot(CELFiles[ ,d],CELFiles[ ,d], xlab="", ylab="",cex=1, main=paste("The correlation value is: ",toString(corr),sep = ""),cex.axis=2,cex.main=2)
